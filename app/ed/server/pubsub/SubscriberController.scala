...
class SubscriberController @Inject()(cc: ControllerComponents, tyCtx: EdContext)
  ...
  def webSocketImpl(request: RequestHeader)
    ...
    var anyOut: Option[SourceQueueWithComplete[JsValue]] = None

    val foreachSink = Sink.foreach[JsValue](v => {
      println(s"Got: $v")
      anyOut.foreach(_.offer(Json.obj("messageEchoed" -> v)))
    })
    ...
    Future.successful(Right(
      Flow.fromSinkAndSourceMat(inSink,  outSource) {
            (_, outboundMat: SourceQueueWithComplete[JsValue]) =>
        outboundMat.offer(JsString("zorro"))
        anyOut = Some(outboundMat)
        //browserConnections ::= outboundMat.offer
        //NotUsed
      }
        // https://stackoverflow.com/a/54137778/694469
        .watchTermination() { (_, doneFuture: Future[Done]) =>
          doneFuture onComplete {
            case Success(_) =>
              println("WebSocket client disconnected")
            case Failure(throwable) =>
              println(s"WebSocket client disconnection failure: ${throwable.getMessage}")
          }
        }
